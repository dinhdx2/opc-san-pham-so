...
const FILE_RE = /[\w\-./]+\.(md|csv|html?|json|js|zip|pdf|xlsx|docx|txt|mp4|mov|webm|mp3|wav|m4a)(\b|$)/i
...
function classifyArtifact(p, headMeta) {
  ...
  if (/(^|\/)(deploy-|kit\/)/.test(base) || /\.(html?|csv|zip|xlsx|docx|svg|png|mp4|mov|webm|mp3|wav|m4a)(\b|$)/i.test(base)) return 'DELIVERABLE'
